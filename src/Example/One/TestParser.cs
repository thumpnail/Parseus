...
public class TestBaseParser : Implicit.BaseParser {
	...
	public override TestRoot Parse(string src) {
		...
		var root = TestRootParser.Parse(new(new BasicAParserContext(lexres.result.ToArray()), new Common.CancellationState()));
		return root;
	}
	// ?#var <ident> = { <ident> : <number> }
	private static Parser<TestRoot> TestRootParser = new((c, self) => {
		Opt(c, tuple => { Literal(c, "var", t => { self.isVar = t; }); });
		Node(c, IdentifierParser, t => { self.identifier = t; });
		Opt(c, (c) => {
			Token(c, TK.equal);
			Token(c, TK.openBracket);
			Opt(c, (c) => {
				Repeat(c, (c) => {
			...
			Token(c, TK.closeingBracket);
		});
	});
	private static Parser<ValuePair> valuePairParser = new((context, self) => {
		Node(context, IdentifierParser, t => { self.Name = t; });
		Token(context, TK.colon);
		Token(context, TK.number, t => { self.Value = t; });
	});
	private static Parser<Identifier> IdentifierParser = new((context, self) => {
		Token(context, TK.identifier, t => { self.value = t; });
	});
}